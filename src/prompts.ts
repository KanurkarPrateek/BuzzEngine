...
export function gateSystem(): string {
  ...
  return [
    ...
    "",
    `Accuracy must be at least ${config.editorial.accuracyThreshold}. Everything else must be at least ${config.editorial.qualityThreshold}.`,
    "The bars differ on purpose. A merely-decent post is worth publishing — do not hold out",
    "for brilliance. But an invented fact published under a real name is the one failure that",
    "matters, so never let 'it needs something to post' soften the accuracy judgement.",
    "",
